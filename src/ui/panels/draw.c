...
static int build_panel_rects(
    ...
    if (state->ui.layout_profile == LAYOUT_PROFILE_QUAD) {
        int top_h = (inner_h * state->ui.layout_sizing.quad_split_y_pct) / 100;
        if (inner_h >= 6) {
            top_h = clamp_int(top_h, 3, inner_h - 3);
        }
        int bottom_h = inner_h - top_h;
        int left_w = (inner_w * state->ui.layout_sizing.quad_split_x_pct) / 100;
        if (inner_w >= 20) {
        ...
        *history = rect_from_slot(state->ui.quad_history_slot, slots);
        *editor = rect_from_slot(state->ui.quad_editor_slot, slots);
        *response = rect_from_slot(state->ui.quad_response_slot, slots);

        return history->ok && editor->ok && response->ok;
    }

    if (state->ui.layout_profile == LAYOUT_PROFILE_FOCUS_EDITOR) {
        int editor_h = (inner_h * state->ui.layout_sizing.focus_editor_height_pct) / 100;
        if (inner_h >= 6) {
    ...
    {
        int left_w = (inner_w * state->ui.layout_sizing.classic_history_width_pct) / 100;
        if (inner_w >= 20) {
            left_w = clamp_int(left_w, 10, inner_w - 10);
        }

        int right_w = inner_w - left_w;
        int right_top_h = (inner_h * state->ui.layout_sizing.classic_editor_height_pct) / 100;
        if (inner_h >= 6) {
...
void ui_draw_init_theme(const AppState *state) {
    g_theme_colors = 0;
    if (!state || !state->ui.theme.use_colors) return;
    if (!has_colors()) return;

    start_color();
    (void)use_default_colors();

    if (init_pair(PAIR_FOCUS, (short)state->ui.theme.focus_fg, (short)state->ui.theme.focus_bg) == ERR) return;
    if (init_pair(PAIR_TAB_ACTIVE, (short)state->ui.theme.tab_active_fg, (short)state->ui.theme.tab_active_bg) == ERR) return;
    if (init_pair(PAIR_GUTTER, (short)state->ui.theme.gutter_fg, (short)state->ui.theme.gutter_bg) == ERR) return;
    if (init_pair(PAIR_WARN, (short)state->ui.theme.warn_fg, (short)state->ui.theme.warn_bg) == ERR) return;
    if (init_pair(PAIR_ERROR, (short)state->ui.theme.error_fg, (short)state->ui.theme.error_bg) == ERR) return;
...
static void draw_history_content(WINDOW *w, const AppState *state) {
    ...
    int start = 0;
    if (state->history.selected >= visible) {
        start = state->history.selected - visible + 1;
    }
    if (start > state->history.history->count - visible) {
        start = state->history.history->count - visible;
    }
    if (start < 0) start = 0;

    for (int i = 0; i < visible; i++) {
        int idx = start + i;
        if (idx >= state->history.history->count) break;

        const HistoryItem *it = &state->history.history->items[idx];

        if (idx == state->history.selected) {
...
static void draw_response_content(WINDOW *w, const AppState *state) {
    ...
    if (state->response.is_request_in_flight) {
        mvwaddnstr(w, 1, 2, i18n_get(state->ui.language, I18N_SENDING_REQUEST), wd - 4);
        wnoutrefresh(w);
        return;
    }

    if (state->response.response.error) {
        if (g_theme_colors) wattron(w, COLOR_PAIR(PAIR_ERROR) | A_BOLD);
        else wattron(w, A_BOLD);
        mvwaddnstr(w, 1, 2, i18n_get(state->ui.language, I18N_REQUEST_FAILED), wd - 4);
        if (g_theme_colors) wattroff(w, COLOR_PAIR(PAIR_ERROR) | A_BOLD);
        else wattroff(w, A_BOLD);

        mvwaddnstr(w, 2, 2, state->response.response.error, wd - 4);
        wnoutrefresh(w);
        return;
    }

    if (!state->response.response.body_view) {
        mvwaddnstr(w, 1, 2, i18n_get(state->ui.language, I18N_NO_RESPONSE_YET), wd - 4);
        wnoutrefresh(w);
        return;
    }
...
static void draw_editor_split_content(WINDOW *w, AppState *state, int *out_cy, int *out_cx) {
    ...
    mvwaddnstr(w, url_label_row, 2, i18n_get(state->ui.language, I18N_URL_LABEL), wd - 4);

    int url_clip = wd - 4;
    if (url_clip < 0) url_clip = 0;
    int url_hscroll = 0;
    if (url_clip > 0 && state->editor.url_cursor >= url_clip) {
        url_hscroll = state->editor.url_cursor - url_clip + 1;
    }

    if (url_hscroll > 0 && url_clip > 0) {
        mvwaddch(w, url_value_row, 2, '<');
    }
    if (state->editor.url_len > url_hscroll + url_clip && url_clip > 1) {
    ...
    if (url_hscroll > 0) {
        display_start++;
        display_width--;
        mvwaddnstr(w, url_value_row, 3, state->editor.url + display_start, display_width - 1);
    } else {
        mvwaddnstr(w, url_value_row, 2, state->editor.url + url_hscroll, url_clip);
    }
    ...
    if (state->editor.active_field == EDIT_FIELD_BODY) {
        if (g_theme_colors) wattron(w, COLOR_PAIR(PAIR_TAB_ACTIVE));
        else wattron(w, A_REVERSE);
    }
    mvwaddnstr(w, split_label_row, body_left, i18n_get(state->ui.language, I18N_BODY_LABEL), body_right - body_left + 1);
    if (state->editor.active_field == EDIT_FIELD_BODY) {
        if (g_theme_colors) wattroff(w, COLOR_PAIR(PAIR_TAB_ACTIVE));
        else wattroff(w, A_REVERSE);
    }

    if (state->editor.active_field == EDIT_FIELD_HEADERS) {
        if (g_theme_colors) wattron(w, COLOR_PAIR(PAIR_TAB_ACTIVE));
        else wattron(w, A_REVERSE);
    }
    mvwaddnstr(w, split_label_row, headers_left, i18n_get(state->ui.language, I18N_HEADERS_LABEL), headers_right - headers_left + 1);
    if (state->editor.active_field == EDIT_FIELD_HEADERS) {
        if (g_theme_colors) wattroff(w, COLOR_PAIR(PAIR_TAB_ACTIVE));
        else wattroff(w, A_REVERSE);
    }
    ...
    draw_textbuffer_area(
        w,
        &state->editor.body,
        &state->editor.body_scroll,
        content_first_row,
        content_last_row,
        body_left,
        body_right,
        state->editor.active_field == EDIT_FIELD_BODY,
        state->editor.active_field == EDIT_FIELD_BODY ? &cy : NULL,
        state->editor.active_field == EDIT_FIELD_BODY ? &cx : NULL
    );

    draw_textbuffer_area(
        w,
        &state->editor.headers,
        &state->editor.headers_scroll,
        content_first_row,
        content_last_row,
        headers_left,
        headers_right,
        state->editor.active_field == EDIT_FIELD_HEADERS,
        state->editor.active_field == EDIT_FIELD_HEADERS ? &cy : NULL,
        state->editor.active_field == EDIT_FIELD_HEADERS ? &cx : NULL
    );

    if (state->editor.active_field == EDIT_FIELD_URL) {
        cy = url_value_row;
        cx = 2 + (state->editor.url_cursor - url_hscroll);
        if (cx < 2) cx = 2;
        if (cx > wd - 2) cx = wd - 2;
    }
...
static void draw_editor_tabs_content(WINDOW *w, AppState *state, int *out_cy, int *out_cx) {
    ...
    int x = 2;
    draw_editor_tab_item(w, 1, x, "URL", state->editor.active_field == EDIT_FIELD_URL);
    x += 7;
    draw_editor_tab_item(w, 1, x, i18n_get(state->ui.language, I18N_BODY_LABEL), state->editor.active_field == EDIT_FIELD_BODY);
    x += 8;
    draw_editor_tab_item(w, 1, x, i18n_get(state->ui.language, I18N_HEADERS_LABEL), state->editor.active_field == EDIT_FIELD_HEADERS);
    ...
    if (state->editor.active_field == EDIT_FIELD_URL) {
        ...
        if (clip < 0) clip = 0;
        int url_hscroll = 0;
        if (clip > 0 && state->editor.url_cursor >= clip) {
            url_hscroll = state->editor.url_cursor - clip + 1;
        }

        mvwaddnstr(w, line_y, 2, state->editor.url + url_hscroll, clip);

        cy = line_y;
        cx = 2 + (state->editor.url_cursor - url_hscroll);
        if (cx < 2) cx = 2;
        if (cx > wd - 2) cx = wd - 2;
    } else if (state->editor.active_field == EDIT_FIELD_BODY) {
        draw_textbuffer_area(
            w,
            &state->editor.body,
            &state->editor.body_scroll,
...
static void draw_editor_content(WINDOW *w, AppState *state) {
    ...
    if (state->ui.mode == MODE_INSERT &&
        state->ui.focused_panel == PANEL_EDITOR &&
        cursor_y >= 0 &&
        cursor_x >= 0) {
        int wy, wx;
        getbegyx(w, wy, wx);
...
void ui_draw(AppState *state) {
    ...
    int layout_changed =
        rows != last_rows ||
        cols != last_cols ||
        state->ui.layout_profile != last_profile ||
        state->ui.quad_history_slot != last_h_slot ||
        state->ui.quad_editor_slot != last_e_slot ||
        state->ui.quad_response_slot != last_r_slot;
    ...
    if (layout_changed) {
        ...
        last_rows = rows;
        last_cols = cols;
        last_profile = state->ui.layout_profile;
        last_h_slot = state->ui.quad_history_slot;
        last_e_slot = state->ui.quad_editor_slot;
        last_r_slot = state->ui.quad_response_slot;
    ...
    snprintf(
        top,
        sizeof(top),
        i18n_get(state->ui.language, I18N_TOPBAR_FMT),
        method_label(state->editor.method),
        layout_profile_name(state->ui.layout_profile)
    );
    ...
    if (state->ui.mode == MODE_COMMAND || state->ui.mode == MODE_SEARCH) {
        char prefix = (state->ui.mode == MODE_COMMAND) ? ':' : '/';
        ...
        if (text_space > 0) {
            ...
            if (state->prompt.cursor >= text_space) {
                prompt_hscroll = state->prompt.cursor - text_space + 1;
            }
            ...
            if (state->prompt.len > prompt_hscroll + text_space && text_space > 1) {
                mvaddch(rows - 1, 4 + text_space - 1, '>');
                display_width--;
            }
            
            if (display_width > 0) {
                mvaddnstr(rows - 1, display_x, state->prompt.input + display_start, display_width);
            }
        }
    } else {
        char status[512];
        int status_warn = 0;
        const char *env_name = env_store_active_name(&state->config.envs);
        int status_x = 2;
        int status_limit_x = cols - 2;

        if (state->search.not_found && state->search.query[0] != '\0') {
            snprintf(
                status,
                sizeof(status),
                i18n_get(state->ui.language, I18N_STATUS_NOT_FOUND_FMT),
                mode_label(state->ui.mode, state->ui.language),
                panel_label(state->ui.focused_panel, state->ui.language),
                env_name ? env_name : i18n_get(state->ui.language, I18N_ENV_NONE),
                state->history.selected,
                state->history.skipped_invalid,
                state->search.query
            );
            status_warn = 1;
        } else {
            snprintf(
                status,
                sizeof(status),
                i18n_get(state->ui.language, I18N_STATUS_DEFAULT_FMT),
                mode_label(state->ui.mode, state->ui.language),
                panel_label(state->ui.focused_panel, state->ui.language),
                env_name ? env_name : i18n_get(state->ui.language, I18N_ENV_NONE),
                state->history.selected,
                state->history.skipped_invalid,
                state->history.last_save_error
            );
        }

        if (state->ui.show_footer_hint && cols > 20) {
            const char *hint = i18n_get(state->ui.language, I18N_HINT_FOOTER);
    ...
    if (w_history && w_editor && w_response) {
        draw_boxed_window(w_history, i18n_get(state->ui.language, I18N_WIN_HISTORY), state->ui.focused_panel == PANEL_HISTORY);
        draw_history_content(w_history, state);

        draw_boxed_window(
            w_editor,
            state->ui.layout_profile == LAYOUT_PROFILE_FOCUS_EDITOR
                ? i18n_get(state->ui.language, I18N_WIN_EDITOR_TABS)
                : (state->editor.active_field == EDIT_FIELD_URL
                       ? i18n_get(state->ui.language, I18N_WIN_EDITOR_URL)
                       : (state->editor.active_field == EDIT_FIELD_BODY
                              ? i18n_get(state->ui.language, I18N_WIN_EDITOR_BODY)
                              : i18n_get(state->ui.language, I18N_WIN_EDITOR_HEADERS))),
            state->ui.focused_panel == PANEL_EDITOR
        );
        draw_editor_content(w_editor, state);

        draw_boxed_window(w_response, i18n_get(state->ui.language, I18N_WIN_RESPONSE), state->ui.focused_panel == PANEL_RESPONSE);
        draw_response_content(w_response, state);
    }

    if (state->ui.mode == MODE_COMMAND || state->ui.mode == MODE_SEARCH) {
        int cx = 4 + state->prompt.cursor;
        if (cx < 2) cx = 2;
        if (cx > cols - 2) cx = cols - 2;
        move(rows - 1, cx);
        curs_set(1);
    } else if (state->ui.mode == MODE_INSERT &&
               state->ui.focused_panel == PANEL_EDITOR &&
